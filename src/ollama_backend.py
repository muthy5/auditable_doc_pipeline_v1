...
def **init**(self, config: OllamaBackendConfig) -> None:
...
def _extract_json_object(self, text: str) -> Dict[str, Any]:
    ...
    # Walk forward from the first '{' using a brace-depth counter
    # to find the matching '}', ignoring braces inside JSON strings.
    start = stripped.find("{")
    if start == -1:
